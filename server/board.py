...
class Board:
    ...
    def randomly_assign_ships(self):
        self.board = self._put_ships_horizontally(Ships.D)
        self.board = self._put_ships_horizontally(Ships.S)
        self.board = self._put_ships_vertically(Ships.C)
        self.board = self._put_ships_vertically(Ships.B)
        self.board = self._put_ships_vertically(Ships.A)
        return self.board

    def shoot(self, x, y, enemy_board):
        is_repeated = False
        if self._is_sea_with_board(x, y, enemy_board):
            enemy_board[y][x] = self.missed_shoot
            return False, is_repeated
        # If you shoot before, that counts as a failure
        elif (
            enemy_board[y][x] == self.missed_shoot
            or enemy_board[y][x] == self.successful_shoot
        ):
            is_repeated = True
            return False, is_repeated
        else:
    ...
    def all_sunken_ships(self, enemy_board):
        for y in range(self.rows):
            for x in range(self.columns):
                ...
                if (
                    cell != SEA
                    and cell != self.successful_shoot
                    and cell != self.missed_shoot
                ):
                    return False
    ...
    def _is_sea(self, x, y):
        return self.board[y][x] == SEA

    def _is_sea_with_board(self, x, y, board):
        return board[y][x] == SEA

    def _put_ships_horizontally(self, enum_boat):
        count = 0
        answer = True
        filled_board = False
        ...
        while filled_board == False:
            answer = True
            x = self._get_random_x()
            y = self._get_random_y()
            x1 = x
            y1 = y
            while count < enum_value:
                if self._is_in_range(x1, y1) and self._is_sea(x1, y1):
                    x1 = x1 + 1
    ...
    def _put_ships_vertically(self, enum_boat):
        count = 0
        answer = True
        filled_board = False
        enum_value = enum_boat.value
...
# uri = daemon.register(Board)
# names_server = Pyro4.locateNS()
# names_server.register('board', uri)
